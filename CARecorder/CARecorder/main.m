...
//  CARecorder
//
//  Created by Michael Fewkes on 9/23/26.
//
...
static int MyComputeRecordBufferSize(const AudioStreamBasicDescription *format, AudioQueueRef queue, float seconds) {
    ...
    if (format->mBytesPerFrame > 0) {
        ...
        if (format->mFramesPerPacket > 0) {
            ...
            // Worst-case scenerio: 1 frame per packet
            packets = frames;
        }
...
int main(int argc, const char * argv[]) {
    ...
    CheckError(MyGetDefaultInputDeviceSampleRate(&recordFormat.mSampleRate), "couldn't get default input device sample rate");
